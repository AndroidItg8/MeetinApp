...
public class TagView extends View {
    ...
    private void init(Context context, Object text) {
    ...
    private void onDealText() {
        ...
        if (!TextUtils.isEmpty(getName())) {
            ...
            mAbstractText = getName().length() <= mTagMaxLength ? getName()
                    : getName().substring(0, mTagMaxLength - 3) + "...";
        } else {
            mAbstractText = "";
        }
    ...
    private String getName() {
        if(mOriginText  instanceof TblTAG)
            return ((TblTAG) mOriginText).getName();
        else if(mOriginText instanceof TblContact)
            return ((TblContact) mOriginText).getName();
        return "";
    }
...
//        }else
//        {
//            mRectF.set(w, w-mBorderWidth,  w, w-mBorderWidth);
//            Log.d(TAG, "onSizeChanged getText().isSelected()  : new width" + w + "new height" + h + "oldw" + oldw + "oldh" + oldh);
//        }
    }
...
//                    (isEnableCross() ? getWidth() - getHeight() :fontW+mHorizontalPadding),
    ...
    public interface OnTagClickListener {
        void onTagClick(int position, Object text);

        void onTagLongClick(int position, Object text);

        void onTagCrossClick(int position, Object text);
    }
}
