...
import static itg8.com.meetingapp.common.CommonMethod.dp2px;
import static itg8.com.meetingapp.common.CommonMethod.sp2px;
...
public class TagContainerLayout extends ViewGroup {
     ...
     */
    private int mBorderColor = Color.parseColor("#FFFFFFFF");
    /**
...
//    private int mTagBackgroundColor = Color.parseColor("#33F44336");
    private int mTagBackgroundColor = Color.parseColor("#12000000");
    /**
...
//    private int mTagTextColor = Color.parseColor("#FF666666");
     ...
     * TagView typeface
     */
    private Typeface mTagTypeface = Typeface.SERIF;
    /**
     ...
     * Tags
     */
    private List<TblContact> mParticipant;
    /**
     ...
     */
    private int mTheme = ColorFactory.PURE_TEAL;
    /**
...
//        int widthSpecMode = MeasureSpec.getMode(widthMeasureSpec);
    ...
    private void onSetTag() {
        ...
        for (int i = 0; i < mTags.size(); i++) {
            tbleTag = mTags.get(i);
            onAddTag(mTags.get(i).getName(), mChildViews.size(), mTags.get(i));
        }
        postInvalidate();
    }

    private void onAddTag(String text, int position, Object tag) {
...
//        tagView.setEnableCross(mEnableCross);
     ...
     * @param colorArrayList
     */
    public void setParticipantTags(List<TblContact> tags, List<int[]> colorArrayList) {
        mParticipant = tags;
        mColorArrayList = colorArrayList;
        onSetParticipantTag();
    ...
    private void onSetParticipantTag() {

        if (mParticipant == null) {
            throw new RuntimeException("NullPointer exception!");
        }
        removeAllTags();
        if (mParticipant.size() == 0) {
            return;
        }
        for (int i = 0; i < mParticipant.size(); i++) {
            tblePartcipant = mParticipant.get(i);
            onAddTag(mParticipant.get(i).getName(), mChildViews.size(), mParticipant.get(i));
        }
        postInvalidate();
    ...
    public void addTag(String text, int position) {
        onAddTag(text, position, tbleTag);
        postInvalidate();
    }
...
//        return ((TagView) mChildViews.get(position)).getText().getName();
//    }
...
//                tmpList.add(((TagView) view).getText().getName());
//            }
//        }
//        return tmpList;
//    }
...
//    }
    ...
    public void changeSelectColor(int position, Object tag) {
